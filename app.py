...
# =============== Import Dataset ==================
df_CustOrderPayment = pd.read_csv('CustomerOrderPayment.csv')
df_ProductAnalysis = pd.read_csv('ProductAnalysis.csv')
...
def main():
    with st.sidebar:
        IDCampImage_path = "ID_CampLogo.jpg"
        st.sidebar.image(IDCampImage_path, use_column_width=True)
        st.text("   ")
    ...
    if selected == 'Dataset Information':
        ...
        local_image_path = "Brazil.png"
        st.text('   ')
        st.image(local_image_path, caption='Brazil Image', use_column_width=True) 
